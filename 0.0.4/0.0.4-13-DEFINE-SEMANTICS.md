...
Defines the **operational semantics** for the **ABT** gate and the **DEF** commit phase, including: [state model, evaluation judgements, pre/post-conditions, error mappings, determinism, object schema]

- This extends and depends on:[operator legality, C-SP routing, MD-flo parity, descent fences, timing & concurrency, and serialization rules previously published]
...
- **Atomic commit**: `Σ, A ⊢ DEF ⇓ D` constructs a single immutable **commit object** `D` (schema in ADS.5) from:
  - `A` and current structural context (nodes, spans, digests)
  - then **appends** `D` to the DEF sink.
...
```json
{
  "$id": "cah-0.0.4-def-commit.schema.json",
  "title": "CAH DEF Commit Object",
  "type": "object",
  "properties": {
    "spec_version": { "const": "0.0.4" },
...
- `commit_object.extensions`: future optional object for provenance signatures and privacy metadata (reserved; absent in 0.0.4).
