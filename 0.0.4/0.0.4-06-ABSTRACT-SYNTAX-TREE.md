...
```json
{
  ...
  "tokens":[ "TOKEN-…" ],
  "flow":[ "FLOW-…" ],        // zero or one FLOW_EXPR nodes; multiple flows must appear as siblings within this array per grammar
  "lanes":[ "LANE-…" ],       // only for center-lane rows with (emo:…:phl)
  "nodes":[ "NODE-…" ]        // CHO/ACT/DEC/ABT/DEF if present in this segment
}
```
...
```json
{
  ...
  "ops":[ "OP-…" ],
  "terms":[ "TERM-…" ],
  "order":[ "OP-…","TERM-…","…" ],   // explicit evaluation order within segment
  "span_group":"G1"                  // identifier grouping open/close pairs (stack discipline)
}
```
...
```json
{
  ...
  "row":1,
  "column":"E_SC|M_SC|P_SC|M_SC",   // where detected
  "glyphs":"raw-slice"              // normalized substring for audit
}
```
...
```json
{
  "$id": "cah-0.0.3-segment-map.schema.json",
  ...
  "$defs": {
    ...
    "flow": {
      ...
      "properties": {
        ...
        "order": { "type": "array", "items": { "type": "string" } }  // ids
      },
