...
*`flow`* holds zero or one `FLOW_EXPR` references per segment; additional flows appear as siblings in the array. *`lanes`* populate only the center band rows that carry `(emo: … :phl)`. *`nodes`* lists any diagram nodes located inside the segment.
