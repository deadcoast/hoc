...
In this schema, `applies_to` enumerates generator identifiers that feed a property, `phase_hint` helps implementers surface lint context (e.g., `P-LAYOUT`), and adapter rules use `selector` strings to address fields within serialized JSON or diagram tokens.
