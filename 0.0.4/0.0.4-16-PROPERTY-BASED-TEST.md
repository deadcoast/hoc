...
```json
{
  "$id": "cah-0.0.3-property-tests.schema.json",
  ...
  "$defs": {
    ...
    "property": {
      ...
      "properties": {
        ...
        "applies_to": { "type": "array", "items": { "type": "string" }, "minItems": 1 },   // generator ids
...
```json
{
  "$id": "cah-0.0.3-lint-catalog.schema.json",
    ...
    "lints": {
      ...
      "items": {
        ...
        "properties": {
          ...
          "phase_hint": { "type": "string" },  // e.g., P-LAYOUT, P-PORTS
...
## ADP — Adapters 0.0.1 / 0.0.2 → 0.0.3 (Lossiness & Remaps)

## ADP.0 Scope

Define machine-readable adapter specifications to transform legacy CAH diagrams/structures (0.0.1, 0.0.2) into 0.0.3-conformant forms or to fail deterministically.

## ADP.1 Adapter Manifest Schema

```json
{
  "$id": "cah-0.0.3-adapter.schema.json",
  "title": "CAH Adapter Manifest",
  "type": "object",
  "properties": {
    "source_version": { "enum": ["0.0.1","0.0.2"] },
    "target_version": { "const": "0.0.3" },
    "rules": {
      "type": "array",
      "items": { "$ref": "#/$defs/rule" },
      "minItems": 1
  ...
  "$defs": {
    "rule": {
      "type": "object",
      "properties": {
        "selector": { "type": "string" },  // path or pattern within serialized_json or diagram tokens
        "action": {
          "enum": [
...
* **Keyword normalization**: rename legacy tokens to 0.0.3 registry (`E-SYN-104` avoidance).
* **Operator upgrades**: map legacy multi-char tokens into 0.0.3 operators (`OPEN_*`, `CLOSE_*`, `BR_*`, `DESC`) or reject.
...
## ADP.4 Error Remapping (legacy → 0.0.3)
...
* After applying rules, the resulting artifact MUST pass **full 0.0.3 validation** or fail with explicit codes; adapters MUST NOT bypass ADS preconditions.
