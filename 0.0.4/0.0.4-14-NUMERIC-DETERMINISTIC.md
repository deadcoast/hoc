...
*`scale`* is the multiplicative factor applied during conversion; *`offset`* allows additive adjustment before scaling, defaulting to zero when omitted.
...
- Validators MUST parse the manifest, and if any feature has `status="required"` but is not listed as `supported=true` in capabilities
  - validation MUST fail early with `E-VERSION-901` (or a dedicated future code)
