...
* `E-PARITY-MISSING-SIDE` (no `ev*` or no `pv*`)
* `E-PARITY-THRESHOLD` (one side below θ)
* `E-PARITY-IMBALANCE` (ratio below ρ)
* `E-PARITY-NORMALIZER` (missing/invalid normalization path)
...
* **Precedence rule** (enforced during validation): `DIAGRAM_SUPREMACY = true`. If derived text conflicts with diagram glyph stream, validator MUST conform to the diagram, flagging `E-PREC-TEXT-DIVERGENCE` as a warning.
