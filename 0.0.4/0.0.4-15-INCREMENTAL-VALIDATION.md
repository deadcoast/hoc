...
- **IR expansion rules (minimal):**
  ...
  2. **Fence influence:** if a dirty row introduces/removes a **TRIPLE** or **FIVE** fence, include the nearest upstream/downstream rows bounded by: node barriers (CHO/ACT/DEC/ABT/DEF) to preserve order
...
`entry` and `exit` refer to stage identifiers within `stages`, marking the pipeline's ingress and egress points.
...
- Redaction applies **pre-serialization** for any exposed JSON.
  - Commit `hash` (ADS.5.1) MUST be computed **after** redaction is applied to the commit payload if redaction is enabled; the unredacted canonical hash MAY also be stored out-of-band.
