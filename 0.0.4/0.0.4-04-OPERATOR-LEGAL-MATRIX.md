...
- A **route** is a sequence of segment positions across rows that preserves column alignment except at C-SP boundaries where an `OPEN_*` or `CLOSE_*` permits crossing and binding.
...
- **Within C-SP (`M_SC`):** `BR_*` MAY redistribute flows horizontally only where the diagram exposes explicit ports; otherwise neutral branches must remain within the originating column.

### T2.3 Aggregation (fan-in)

- When multiple flows converge in `M_SC`, the merge operation is a deterministic union of payload bindings by identifier while preserving side tags.
- Duplicate identifiers coalesce; ordering after aggregation carries no semantic weight.

### T2.4 Fan-out

- Neutral `BR_*` MAY duplicate routing state; payload references remain aliases and do not clone bindings. Parity evaluation observes the original bindings.

### T2.5 Conflicts

- Contradictory column claims (for example an `EV_ID` observed on `P_SC`) constitute a type error and invalidate the route segment.
...
- `register_normalizer(NormalizerID, Domain, Unit, Policy, Params, Version)` → `OK|E-NORM-A01`
- `resolve_normalizer(EV_ID|PV_ID)` → `NormalizerID|E-NORM-A02`
...
- Missing mapping triggers `E-NORM-A02` (parity phase surfaces `E-PARITY-504` when unresolved bindings persist).
- Unit mismatch (`unit` not equal to the normalizer’s expected unit) triggers `E-NORM-A03`.
