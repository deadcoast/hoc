...
`constraints` may hold structured policy references for automated review, while each equivalence `policy` names the handling strategy (for example `free-order-warns`). For badge issuance, `issuer.pubkey` supplies a PEM or JWK thumbprint string, and `signature.sig` carries a base64-encoded signature payload.
