...
`constraints` may hold structured policy references for automated review, while each equivalence `policy` names the handling strategy (for example `free-order-warns`).
For badge issuance, `issuer.pubkey` supplies a PEM or JWK thumbprint string, and `signature.sig` carries a base64-encoded signature payload.
...
- **Level C**: `RPP.conformance_level==LEVEL_C`, pass **all** CTM tests including PREC/VERSION, and round-trip RCA checks;
  - `summary.failed==0`, `summary.errors==0`, and any `deviations` with `area ∈ {"ROUTE","SPAN","PARITY","ABT","DEF","SERIALIZE","DIGEST"}` MUST be absent.
