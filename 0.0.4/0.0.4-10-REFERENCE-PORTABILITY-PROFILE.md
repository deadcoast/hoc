...
```json
{
  "$id": "cah-0.0.3-portability-profile.schema.json",
...
* `CAH-0.0.3-A` — Authoring compliance (RPP LEVEL_A).
* `CAH-0.0.3-B` — Integration compliance (RPP LEVEL_B).
* `CAH-0.0.3-C` — Commit compliance (RPP LEVEL_C).

## CBS.3 Badge Artifact (JSON, verifiable)

```json
{
  "$id": "cah-0.0.3-compliance-badge.schema.json",
  "title": "CAH Compliance Badge",
  "type": "object",
  "properties": {
    "series": { "const": "CAH" },
    "level": { "enum": ["CAH-0.0.3-A","CAH-0.0.3-B","CAH-0.0.3-C"] },
    "issuer": {
      "type": "object",
      "properties": {
        "name": { "type": "string" },
    ...
    "rpp": { "$ref": "cah-0.0.3-portability-profile.schema.json#" },
    "evidence": {
      "type": "object",
      "properties": {
