...
## T5 — Serialization Schema (Canonical)

### T5.1 Envelope

* `version`: string, exactly `"0.0.4"`.
* `diagram_digest`: 64-character hex SHA-256 of the normalized diagram glyph stream.
* `nodes`: ordered array of node records (see T5.2).
* `spans`: ordered array of span records (see T5.3).
* `payloads`: side-keyed identifier table (see T5.4).
* `validation`: optional summary object during authoring; REQUIRED for committed artifacts.

### T5.2 Node record

* `id`: stable node identifier (e.g., `CHO-1`, `DEF-1`).
* `column`: `E_SC`, `M_SC`, or `P_SC` (`DEF` is always `M_SC`).
* `row`: integer (1-based) after frame stripping.

### T5.3 Span record

* `span_id`: integer assigned in creation order.
* `side`: `LEFT` or `RIGHT`.
* `open`: `{row, column, offset}` coordinates of the opener.
* `close`: `{row, column, offset}` coordinates of the closer.
* `payload_refs`: `{emo: [EV_ID…], phl: [PV_ID…]}` references accrued while the span is open.
* `route_ok`: boolean summarizing legality checks.

### T5.4 Payloads

* `emo`: object mapping `EV_ID` → `{scale, conf, dir}` triplets.
* `phl`: object mapping `PV_ID` → `{scale, conf, dir}` triplets.

### T5.5 Canonical ordering & hashing

* Sort object keys lexicographically; maintain array order by insertion index.
* Compute `diagram_digest` over the RCA-normalized glyph stream (frame stripped, whitespace canonicalized per T9).
* Re-rendering the serialization MUST reproduce the same digest; mismatches raise `E-PREC-801`.

---

## MCC — Minimal Compliance Checklist (0.0.4)

**Purpose:** Establish the smallest set of requirements an implementation MUST satisfy to claim CAH 0.0.4 compliance.
